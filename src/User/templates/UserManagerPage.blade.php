...
@section('contents')
    ...
    <table
            ...
            data-edit-csrf-token="{{ $tokenHandler->get('user', 'edit') }}"
            data-resetpassword-csrf-token="{{ $tokenHandler->get('user', 'resetpassword') }}"
            data-delete-csrf-token="{{ $tokenHandler->get('user', 'delete') }}">
        <thead>
            <tr>
                <th>ID</th>
    ...
    @component('View/Widget/Modal', ['id' => 'um-update-avatar-dialog', 'title' => 'Avatar aanpassen', 'sizeClass' => 'modal-lg'])
        @slot('body')
            <form id="um-update-avatar" method="post" action="/user/changeAvatar/" enctype="multipart/form-data">
                <input type="hidden" name="csrfToken" value="{{ $tokenHandler->get('user', 'changeAvatar') }}"/>
