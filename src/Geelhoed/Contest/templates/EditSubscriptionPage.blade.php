...
@section ('contents')
    ...
    <form method="post" action="/contest/editSubscription/{{ $contestMember->id }}">
        ...
        @endcomponent
        @include('Widget/Form/Number', ['id' => 'weight', 'label' => 'Gewicht', 'required' => true, 'value' => $contestMember->weight])
        @include('Widget/Form/Select', ['id' => 'graduationId', 'label' => 'Band', 'required' => true, 'selected' => $contestMember->graduationId, 'options' => $graduations])
